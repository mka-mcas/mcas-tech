...
/* ─── DATA MATRIX STRUCTURE WITH SYSTEM CITATIONS ────────────────── */
    ...
    levels: [
      {
        ...
        data2: [
          { name: "Clear sky", value: 93.0, color: "#22C55E" },
          { name: "Rain / Wet", value: 5.4, color: "#818CF8" },
          { name: "Foggy haze", value: 0.9, color: "#6B7280" },
          { name: "Other mix", value: 0.7, color: "#374151" },
        ],
      },
      {
    ...
    levels: [
      {
        ...
        data: [
          ...
          { name: "Bus transit", value: 2, color: "#22C55E" },
          { name: "Other profile", value: 1, color: "#6B7280" },
        ],
      },
      {
    ...
    levels: [
      {
        ...
        data: [
          { name: "Male", value: 92.1, color: "#818CF8" },
          { name: "Female", value: 7.9, color: "#F472B6" },
        ],
        data2: [
          { name: "Rider Operator", value: 88.5, color: "#F59E0B" },
          { name: "Pillion Passenger", value: 11.5, color: "#6B7280" },
        ],
      },
      {
        ...
        data: [
          ...
          { name: "41–50 years", value: 9.9, color: "#EAB308" },
          { name: "51–60 years", value: 9.1, color: "#84CC16" },
          { name: "60+ elderly", value: 9.1, color: "#22C55E" },
          { name: "31–35 years", value: 6.4, color: "#22C55E" },
          { name: "36–40 years", value: 4.8, color: "#14B8A6" },
          { name: "11–15 youth", value: 4.7, color: "#06B6D4" },
        ],
      },
      {
        title: "Machine Ownership Matrix",
        insight:
          "9 out of 10 fatal impacts happen on an asset registered under personal/private ownership metadata. The vehicle a rider operates for routine daily logistics holds the greatest absolute proximity threat window.",
        type: "pie",
        data: [
          { name: "Private / Personal", value: 90, color: "#F59E0B" },
          { name: "Commercial transit", value: 6, color: "#6B7280" },
    ...
    levels: [
      {
        ...
        type: "pie",
        data: [
          { name: "Properly Fastened", value: 76, color: "#22C55E" },
    ...
    levels: [
      {
        ...
        insight:
          "Head trauma remains the primary driver of mortality indices at 63%. Because brain tissue exhibits zero tolerance for terminal deceleration forces, protective shell validation remains non-negotiable for every single run.",
        type: "pie",
        data: [
          ...
          { name: "Lower Extremities", value: 3, color: "#22C55E" },
...
const PieLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, value }) => {
  ...
  return (
    <text x={x} y={y} fill="#000" textAnchor="middle" dominantBaseline="central" className="text-[11px] font-black">
      {value}%
    </text>
  );
};
...
const CustomChartTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-3 shadow-2xl text-xs font-mono">
      <p className="text-zinc-400 font-bold mb-1">{data.name || data.payload?.name}</p>
      <p className="font-black text-sm" style={{ color: data.fill || data.color || "#FBBF24" }}>
  ...
  return (
    <div className="flex gap-6 items-center flex-wrap justify-center sm:justify-start">
      ...
      <div className="flex flex-col gap-2 flex-1 min-w-[240px]">
        {data.map((item, idx) => (
          <div key={idx} className="flex items-center justify-between text-xs font-mono border-b border-zinc-800/40 pb-1.5 last:border-0">
    ...
    <div className="flex flex-col gap-3 font-mono">
      {data.map((item, idx) => (
        <div key={idx} className="flex items-center gap-4 text-xs">
          <div className="text-zinc-500 text-right font-bold truncate w-[130px] sm:w-[160px]" title={item.name}>
            {item.name}
          </div>
          <div className="flex-1 h-7 bg-zinc-950 rounded-lg border border-zinc-900 overflow-hidden relative">
            <div
    ...
    <div className="w-full">
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={data} margin={{ top: 12, right: 10, left: -25, bottom: 20 }}>
          <XAxis dataKey="name" tick={{ fill: "#52525b", fontSize: 10, fontFamily: 'monospace' }} interval={0} stroke="#18181b" />
          <YAxis tick={{ fill: "#3f3f46", fontSize: 10, fontFamily: 'monospace' }} tickFormatter={v => `${v}%`} stroke="#18181b" />
          <Tooltip content={<CustomChartTooltip />} />
          <Bar dataKey="value" radius={[4, 4, 0, 0]}>
            {data.map((entry, index) => (
              <Cell key={`cell-${index}`} fill={entry.value >= 11 ? "#EF4444" : entry.value >= 8 ? "#F59E0B" : "#27272a"} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
      <div className="flex flex-wrap justify-center gap-6 mt-4 text-[10px] font-mono tracking-wider uppercase text-zinc-500 border-t border-zinc-800/40 pt-4">
        <span className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-sm bg-[#EF4444]" /> Critical Threat Vector (≥11%)
        </span>
        <span className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-sm bg-[#F59E0B]" /> Elevated Hazard (8%–11%)
        </span>
        <span className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-sm bg-[#27272a]" /> Baseline Vector (&lt;8%)
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
      <div className="bg-zinc-950/40 border border-zinc-900 p-4 rounded-xl">
        <p className="text-[10px] font-mono font-bold tracking-widest text-zinc-500 uppercase mb-4 text-center">
...
export default function RiskInfographic() {
  ...
  // Hook layout validation block to safely suppress server/client dynamic hydration deltas
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  if (!mounted) {
    return (
      <div className="w-full bg-[#060813] text-zinc-400 min-h-[400px] flex items-center justify-center font-mono text-xs">
  ...
  if (!selectedId) {
    return (
      <div className="bg-[#060813] text-zinc-100 min-h-screen p-6 font-sans border border-zinc-900 rounded-3xl shadow-2xl relative overflow-hidden">
        
        {/* Dynamic Abstract Branding Grid Accent lines */}
        <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-gradient-to-bl from-sky-500/5 via-transparent to-transparent pointer-events-none rounded-full blur-3xl" />

        {/* Analytical Heading Block */}
        <div className="text-center mb-12 border-b border-zinc-900 pb-8">
          <p className="text-[10px] font-mono tracking-widest text-zinc-500 uppercase mb-3">
            EPIDEMIOLOGICAL EXPOSURE SURVEY // MALAYSIAN ROYAL POLICE DATA ARCHIVE [cite: 2475, 2540]
          </p>
          <h2 className="text-3xl sm:text-5xl font-black tracking-tight uppercase leading-none text-white">
            ...
            <span className="font-light text-zinc-500 lowercase text-xl sm:text-3xl font-mono block mt-2">
              in macro-analytics perspective [cite: 2460]
            </span>
          </h2>
          <p className="text-zinc-500 text-xs mt-3 max-w-xl mx-auto leading-relaxed">
            Parsing instrumented road casualty logs to expose systemic behavior flaws. Click any dynamic category vector below to process data drill levels. [cite: 2475, 2537]
          </p>
        </div>

        {/* ── Macro Telemetry Vital Signs Banner ── */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto mb-12 bg-zinc-950/60 p-5 rounded-2xl border border-zinc-900">
          {[
            { value: "4,070", label: "fatalities in peak baseline year [cite: 2491]", color: "text-rose-500" },
            { value: "60%",   label: "of all macro Malaysian road deaths [cite: 2474, 2490]", color: "text-amber-400" },
            { value: "#1 Risk", label: "ASEAN fatality incidence per capita [cite: 2474, 2602]", color: "text-indigo-400" },
            { value: "8 / Day", label: "mean mortal velocity frequency [cite: 2737]", color: "text-emerald-400" },
          ].map((stat, i) => (
            <div key={i} className="text-center border-r border-zinc-900/60 last:border-0 px-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-5xl mx-auto">
          {Object.values(CATEGORIES).map(item => (
            <button
              ...
              className={`${item.tailwindBg} border ${item.tailwindBorder} rounded-2xl p-6 text-left transition-all duration-300 hover:-translate-y-1 hover:bg-zinc-900/20 group outline-none relative overflow-hidden`}
            >
              <div className="text-3xl mb-4 bg-zinc-950 w-12 h-12 rounded-xl flex items-center justify-center border border-zinc-900 group-hover:border-zinc-800 transition-all">
                {item.icon}
              </div>
              <span className={`text-[10px] font-mono tracking-widest uppercase block mb-1 ${item.tailwindText}`}>
                {item.sub}
              </span>
              <h4 className="text-white font-black text-lg mb-2 tracking-tight uppercase">
                {item.label}
              </h4>
              <p className="text-zinc-400 text-xs leading-relaxed border-t border-zinc-900/80 pt-3 font-medium min-h-[50px]">
                {item.headline}
              </p>
        ...
        <div className="text-center mt-12 text-[10px] font-mono text-zinc-600">
          Source Reference: Manan &amp; Várhelyi (2012) // International Association of Traffic and Safety Sciences [cite: 2457, 2461]
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#060813] text-zinc-100 min-h-screen p-6 font-sans border border-zinc-900 rounded-3xl shadow-2xl">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6 border-b border-zinc-900 pb-4">
          <button
            ...
            className="bg-zinc-950 hover:bg-zinc-900 border border-zinc-800 text-zinc-300 font-mono font-bold text-xs px-4 py-2 rounded-xl transition-all"
          >
            ← System Back
          </button>
          <div className="text-[11px] font-mono text-zinc-500 truncate max-w-md">
            <button onClick={() => { setSelectedId(null); }} className="hover:text-zinc-300">
              Console
            </button>
            ...
            <span className="mx-2 text-zinc-700">/</span>
            <span className="text-zinc-400">{currentLevel.title}</span>
          </div>
        </div>
        ...
        <div className="flex gap-2 mb-8">
          {cat.levels.map((l, i) => (
            <button
              ...
              style={{ background: i <= level ? cat.accentHex : "#18181b" }}
            />
          ))}
        </div>
        ...
        <div className="mb-6 space-y-4">
          ...
          <h3 className="text-2xl sm:text-4xl font-black tracking-tight text-white uppercase leading-none">
            {currentLevel.title}
          </h3>
          <div 
            className="rounded-2xl p-5 border text-xs sm:text-sm leading-relaxed"
            style={{
              background: `${cat.accentHex}06`,
              borderColor: `${cat.accentHex}20`
            }}
          >
            ...
            <span className="text-zinc-300 font-medium">{currentLevel.insight}</span>
          </div>
        </div>

        {/* Main Graphical Presentation Block Section */}
        <div className="bg-zinc-950/40 border border-zinc-900 rounded-2xl p-6 mb-6 shadow-xl relative min-h-[220px] flex flex-col justify-center">
          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            {cat.levels.map((l, i) => (
              <button
                ...
                style={
                  i === level
                    ? { background: cat.accentHex, borderColor: cat.accentHex, color: "#000" }
                    : { background: "#0c0f1d", borderColor: "#18181b", color: "#52525b" }
                }
              >
                Layer {i + 1}
              </button>
        ...
        {level < cat.levels.length - 1 ? (
          <button
            ...
            className="w-full rounded-2xl py-4 text-xs font-mono font-bold uppercase tracking-widest transition-opacity hover:opacity-95 shadow-lg outline-none"
          ...
          <button
            ...
            className="w-full bg-zinc-950 hover:bg-zinc-900 text-zinc-400 border border-zinc-800 rounded-2xl py-4 text-xs font-mono font-bold uppercase tracking-widest transition-colors outline-none"
          >
            ← Terminate Drill Map // Return to Console Home
          </button>
        )}

        <div className="text-center mt-8 text-[9px] font-mono text-zinc-700 tracking-tight">
          Incident Dataset sourced from Royal Malaysia Police records &amp; MIROS repository matrices[cite: 2475, 2538, 2540].
        </div>
      </div>
    </div>
  );
